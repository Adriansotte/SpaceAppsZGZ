...
def process_file(row):
    try:
        ...
        # Extraer los datos de tiempo relativo y velocidad (convertir de c/s a m/s dividiendo entre 100)
        csv_times = np.array(data_cat['time_rel(sec)'].tolist())
        csv_data = np.array(data_cat['velocity(c/s)'].tolist()) / 100  # Convertir de c/s a m/s
...
results = Parallel(n_jobs=num_jobs)(delayed(process_file)(row) for index, row in cat.iterrows())
